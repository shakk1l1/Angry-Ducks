...
abstract class Objet(
    ...
    open fun update2(interval:Double){                  //Entrées:intervalle , Sorties:None
        if(onscreen) {                                  //Actualise la position de l'objet en intégrant sa vitesse sur l'intervalle de temps
            ...
            vitessey += (interval * 1000.0f).toFloat()    // la "gravité"
        }
    }

    fun collisionSphereSphere(x1:Double,y1:Double,r1:Double,x2:Double,y2:Double,r2:Double) {
        val one = ((x1-x2).pow(2)+(y1-y2).pow(2)).pow(0.5)  //Entrées:position x, y de l'objet 1 et 2 et leurs rayons , Sorties:None
        val two = r1+r2                                              //Détecte les collisions entre les objets en calculant leur distance et en la comparant à leur rayon
        colliding =(one<two)

    }
    fun sphereCollideSphere(v1x:Double,v1y:Double,m1:Double,v2x:Double,v2y:Double,m2:Double,coef:Double, objet: Objet) {
        val vmoyx:Double = (m1*v1x+m2*v2x)/(m1+m2)          //Entrées: vitesse x et y des 2 objets ainsi que leurs masses respectives, Sorties:None
        val vmoyy:Double = (m1*v1y+m2*v2y)/(m1+m2)          //Calcule les vitesses finale des 2 objets qui se collisionnent, modifie la vitesse de l'objet exécutat la méthode
        val dv1x=(1.0+coef)*(v1x-vmoyx)                     //et appelle le deuxième objet pour qu'il change lui même sa vitesse
    ...
    fun touchinggrass(): Boolean {                              //Entrées:None , Sorties:retourne si l'objet concerné touche le sol du niveau
